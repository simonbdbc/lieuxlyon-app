  ...
  methods: {
    loadLeaflet() {
      ...
      function calculPointIntersectionLatitude(aCoord, sLatitude) {
        ...
        var lon = Math.atan(
          (Math.sin(lon1) * Math.cos(lon2) * Math.sin(lat - lat2) -
            Math.sin(lon2) * Math.cos(lon1) * Math.sin(lat - lat1)) /
            (Math.cos(lon1) * Math.cos(lon2) * Math.sin(lat1 - lat2))
        );
